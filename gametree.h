...
private:
    GameTree(uint32_t maxDepth, uint8_t radius, int iterations, double exploration,uint8_t (&board)[15][15]);
    // MCTS 4步法：选择/扩展/模拟/回溯
    Node* select(Node* node);
    Node* expand(Node* node, char choice);
    double simulate(Node* node, char choice);
    ...
    // MCTS参数
    int maxIterations = 1e5/2;  // 最大迭代次数1e5/2=50000
    double exploration = 2; // 探索参数
