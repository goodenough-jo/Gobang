...
vector<pair<uint8_t, uint8_t>> goback; //存放回溯需要的点的坐标
int main()
{
    //选择先手或者后手
...
void designatedStartP()
{
    ...
    if (answer) {
        ...
        for (uint8_t k = 0; k < 220; k++) {
            ...
            GameTree gt = GameTree(9, 2, board); //每循环一次复制一颗博弈树
    ...
    for (uint8_t k = 2; k < 225; k++) {
        if (i != 0 && i != 1) //非第四步和第五步 从第7步输入黑方坐标开始
        {
            ...
            if (_x == 'Q') {
                ...
                GameTree gtGoback = GameTree(9, 2, board);
        ...
        else if (i == 1) //第5步
        {
            ...
            for (unsigned long k = 0; k < needSelection.size(); k++) {
                ...
                //调用评估函数 得到当前最佳子节点的估值
                GameTree gtSix = GameTree(5, 2, board); //------
                gtSix.game(1, 1, 1);
